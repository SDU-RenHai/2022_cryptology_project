...
static inline WORD SIMD_print(__m256i mm_val, size_t pos) {
	WORD temp[8];
	_mm256_storeu_epi32(temp, mm_val);
	return temp[pos];
}
...
static inline __m256i SIMD_word_rotate_sll (__m256i mm_val, size_t imm) {
	...
	return _mm256_or_epi32(mm_sll, mm_srl);
}
...
static inline __m256i SIMD_P0(__m256i mm_val) {
	...
	return _mm256_xor_epi32(mm_val, _mm256_xor_epi32(mm_sll9, mm_sll17));
}
...
static inline __m256i SIMD_P1(__m256i mm_val) {
	...
	return _mm256_xor_epi32(mm_val, _mm256_xor_epi32(mm_sll15, mm_sll23));
}

static inline __m256i SIMD_FF_0_15(__m256i A, __m256i B, __m256i C) {
	return _mm256_xor_epi32(_mm256_xor_epi32(A, B), C);
}

static inline __m256i SIMD_FF_16_63(__m256i A, __m256i B, __m256i C) {
	__m256i X = _mm256_and_epi32(A, B);
	__m256i Y = _mm256_and_epi32(A, C);
	__m256i Z = _mm256_and_epi32(B, C);
	return _mm256_or_epi32(X, _mm256_or_epi32(Y, Z));
}
...
static inline __m256i SIMD_GG_16_63(__m256i A, __m256i B, __m256i C) {
	__m256i X = _mm256_and_epi32(A, B);
	__m256i Y = _mm256_andnot_epi32(A, C);
	return _mm256_or_epi32(X, Y);
}
...
void SM3_SIMD_Engine::msg_expansion()
{
	...
	for (size_t i = 16; i < 68; ++i) {
		...
		__m256i X = _mm256_loadu_epi32(W[i - 16].data());
		__m256i mm_w_i9 = _mm256_loadu_epi32(W[i - 9].data());
		X = _mm256_xor_epi32(X, mm_w_i9);
		__m256i mm_w_i3 = _mm256_loadu_epi32(W[i - 3].data());
		mm_w_i3 = SIMD_word_rotate_sll(mm_w_i3, 15);
		X = _mm256_xor_epi32(X, mm_w_i3);

		// WORD Y = WORD_ROTATE_LEFT(W[i - 13], 7);
		__m256i Y = _mm256_loadu_epi32(W[i - 13].data());
		Y = SIMD_word_rotate_sll(Y, 7);

		// this->W[i] = P1(X) ^ Y ^ W[i - 6];
		__m256i ANS = _mm256_loadu_epi32(W[i - 6].data());
		ANS = _mm256_xor_epi32(ANS, Y);
		ANS = _mm256_xor_epi32(ANS, SIMD_P1(X));

		// Move to W;
		_mm256_storeu_epi32(W[i].data(), ANS);
	...
	for (size_t i = 0; i < 64; ++i) {
		...
		__m256i ANS = _mm256_loadu_epi32(W[i].data());
		__m256i wi_4 = _mm256_loadu_epi32(W[i + 4].data());
		ANS = _mm256_xor_epi32(ANS, wi_4);

		// Move to W_stroke;
		_mm256_storeu_epi32(W_stroke[i].data(), ANS);
...
void SM3_SIMD_Engine::compress()
{
	...
	__m256i mmA = _mm256_loadu_epi32(IV[0].data());
	__m256i mmB = _mm256_loadu_epi32(IV[1].data());
	__m256i mmC = _mm256_loadu_epi32(IV[2].data());
	__m256i mmD = _mm256_loadu_epi32(IV[3].data());
	__m256i mmE = _mm256_loadu_epi32(IV[4].data());
	__m256i mmF = _mm256_loadu_epi32(IV[5].data());
	__m256i mmG = _mm256_loadu_epi32(IV[6].data());
	__m256i mmH = _mm256_loadu_epi32(IV[7].data());
	...
	_mm256_storeu_epi32(IV[0].data(), _mm256_xor_epi32(mmA, _mm256_loadu_epi32(IV[0].data())));
	_mm256_storeu_epi32(IV[1].data(), _mm256_xor_epi32(mmB, _mm256_loadu_epi32(IV[1].data())));
	_mm256_storeu_epi32(IV[2].data(), _mm256_xor_epi32(mmC, _mm256_loadu_epi32(IV[2].data())));
	_mm256_storeu_epi32(IV[3].data(), _mm256_xor_epi32(mmD, _mm256_loadu_epi32(IV[3].data())));
	_mm256_storeu_epi32(IV[4].data(), _mm256_xor_epi32(mmE, _mm256_loadu_epi32(IV[4].data())));
	_mm256_storeu_epi32(IV[5].data(), _mm256_xor_epi32(mmF, _mm256_loadu_epi32(IV[5].data())));
	_mm256_storeu_epi32(IV[6].data(), _mm256_xor_epi32(mmG, _mm256_loadu_epi32(IV[6].data())));
	_mm256_storeu_epi32(IV[7].data(), _mm256_xor_epi32(mmH, _mm256_loadu_epi32(IV[7].data())));
}
